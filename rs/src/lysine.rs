...
// Main point of inlysinection in this library.
///
// The [`Lysine`] struct is the primary interface for working with the Lysine template engine. It contains parsed templates, registered filters (which can filter
...
// use lysine::Lysine;
///
// // Create a new Lysine instance and add a template from a string
// let mut lysine = Lysine::new("templates/**/*").unwrap();
// lysine.add_raw_template("hello", "Hello, {{ name }}!").unwrap();
///
...
// let rendered = lysine.render("hello", &context).unwrap();
...
impl Lysine {
    ...
    // Create a new instance of Lysine, containing all the parsed templates found in the `dir` glob.
    ///
    ...
    ///
    // In order to create an empty [`Lysine`] instance, you can use the [`Default`] implementation.
    ///
    // # Examples
    ///
    // Basic usage:
    ...
    // ```no_run
    // # use lysine::Lysine;
    // let lysine = Lysine::new("examples/basic/templates/**/*").unwrap();
    // ```
    ...
    // Create a new instance of Lysine, containing all the parsed templates found in the `dir` glob.
    ///
    // The difference to [`Lysine::new`] is that it won't build the inheritance chains
    // automatically, so you are free to modify the templates if you need to.
    ///
    // # Inheritance Chains
    ///
    // You will *not* get a working Lysine instance using this method. You will need to call
    // [`build_inheritance_chains()`](Lysine::build_inheritance_chains) to make it usable.
    ///
    // # Examples
    ///
    // Basic usage:
    ...
    // ```no_run
    // # use lysine::Lysine;
    // let mut lysine = Lysine::parse("examples/basic/templates/**/*").unwrap();
    ///
    ...
    // Renders a Lysine template given a [`Context`].
    ///
    // # Examples
    ///
    // Basic usage:
    ///
    // ```
    // # use lysine::{Lysine, Context};
    // // Create new lysine instance with sample template
    // let mut lysine = Lysine::default();
    // lysine.add_raw_template("info", "My age is {{ age }}.");
    ///
    // // Create new context
    ...
    // let output = lysine.render("info", &context).unwrap();
    ...
    // ```
    // # use lysine::{Lysine, Context};
    // // Create new lysine instance with demo template
    // let mut lysine = Lysine::default();
    // lysine.add_raw_template("hello.html", "<h1>Hello</h1>");
    ///
    // // Render a template with an empty context
    // let output = lysine.render("hello.html", &Context::new()).unwrap();
    ...
    // Renders a Lysine template given a [`Context`] to something that implements [`Write`].
    ///
    ...
    // let mut lysine = Lysine::default();
    // lysine.add_raw_template("index.html", "<p>{{ name }}</p>");
    ///
    // // Rendering a template to an internal buffer
    // let mut buffer = Vec::new();
    // let mut context = Context::new();
    // context.insert("name", "John Wick");
    // lysine.render_to("index.html", &context, &mut buffer).unwrap();
    ...
    // input) given a `Context` and an instance of Lysine. This allows you to
    // render templates using custom filters or functions.
    ///
    // Any errors will mention the `__lysine_one_off` template: this is the name
    // given to the template by Lysine.
    ///
    // ```no_compile
    // let mut context = Context::new();
    // context.insert("greeting", &"Hello");
    // let string = lysine.render_str("{{ greeting }} World!", &context)?;
    ...
    ///
    // This creates a separate instance of Lysine with no possibilities of adding custom filters
    // or testers, parses the template and render it immediately.
    // Any errors will mention the `__lysine_one_off` template: this is the name given to the template by
    // Lysine
    ///
    // ```no_compile
    // let mut context = Context::new();
    // context.insert("greeting", &"hello");
    // Lysine::one_off("{{ greeting }} world", &context, true);
    // ```
    ...
    ///
    // let mut lysine = Lysine::default();
    // lysine.add_raw_template("foo", "{{ hello }}");
    // lysine.add_raw_template("another-one.html", "contents go here");
    ///
    // let names: Vec<_> = lysine.get_template_names().collect();
    ...
    // Add a single template to the Lysine instance.
    ///
    ...
    // [`add_raw_templates()`](Lysine::add_raw_templates).
    ///
    // # Examples
    ///
    // Basic usage:
    ...
    // # use lysine::Lysine;
    // let mut lysine = Lysine::default();
    // lysine.add_raw_template("new.html", "Blabla").unwrap();
    // ```
    ...
    // Add all the templates given to the Lysine instance
    ///
    // This will error if the inheritance chain can't be built, such as adding a child
    // template without the parent one.
    ///
    ...
    // Add a single template from a path to the Lysine instance. The default name for the template is
    // the path given, but this can be renamed with the `name` parameter
    ///
    // This will error if the inheritance chain can't be built, such as adding a child
    // template without the parent one.
    // If you want to add several file, use [Lysine::add_template_files](struct.Lysine.html#method.add_template_files)
    ///
    // ```
    // # use lysine::Lysine;
    // let mut lysine = Lysine::default();
    // // Rename template with custom name
    // lysine.add_template_file("examples/basic/templates/macros.html", Some("macros.html")).unwrap();
    // // Use path as name
    // lysine.add_template_file("examples/basic/templates/base.html", None).unwrap();
    // ```
    ...
    // Add several templates from paths to the Lysine instance.
    ///
    ...
    // # use lysine::Lysine;
    // let mut lysine = Lysine::default();
    // lysine.add_template_files(vec![
    //     ("./path/to/template.lysine", None), // this template will have the value of path1 as name
    //     ("./path/to/other.lysine", Some("hey")), // this template will have `hey` as name
    // ]);
    // ```
    ...
    // Register a filter with Lysine.
    ///
    // If a filter with that name already exists, it will be overwritten
    ///
    // ```no_compile
    // lysine.register_filter("upper", string::upper);
    // ```
    ...
    // Register a tester with Lysine.
    ///
    // If a tester with that name already exists, it will be overwritten
    ///
    // ```no_compile
    // lysine.register_tester("odd", testers::odd);
    // ```
    ...
    // Register a function with Lysine.
    ///
    ...
    ///
    // ```no_compile
    // lysine.register_function("range", range);
    // ```
    ...
    // lysine.autoescape_on(vec![".php.html"]);
    // // disable autoescaping completely
    // lysine.autoescape_on(vec![]);
    // ```
    ...
    // ```
    // # use lysine::{Lysine, Context};
    // // Create new Lysine instance
    // let mut lysine = Lysine::default();
    ///
    // // Override escape function
    // lysine.set_escape_fn(|input| {
    //     input.escape_default().collect()
    // });
    ///
    // // Create template and enable autoescape
    // lysine.add_raw_template("hello.js", "const data = \"{{ content }}\";").unwrap();
    // lysine.autoescape_on(vec!["js"]);
    ///
    ...
    ///
    // // Render template
    // let result = lysine.render("hello.js", &context).unwrap();
    ...
    // Re-parse all templates found in the glob given to Lysine.
    ///
    ...
    // Extend this [`Lysine`] instance with the templates, filters, testers and functions defined in
    // another instance.
    ///
    // Use that method when you want to add a given Lysine instance templates/filters/testers/functions
    ...
    // // except the ones that have an identical name to the ones in `my_lysine`
    // my_lysine.extend(&FRAMEWORK_TERA);
    ///```
...
mod tests {
    ...
    fn test_get_inheritance_chain() {
        ...
        assert_eq!(
            lysine.get_template("a").unwrap().parents,
            vec!["b".to_string(), "c".to_string(), "d".to_string()]
        );
        assert_eq!(lysine.get_template("b").unwrap().parents, vec!["c".to_string(), "d".to_string()]);
        assert_eq!(lysine.get_template("c").unwrap().parents, vec!["d".to_string()]);
        assert_eq!(lysine.get_template("d").unwrap().parents.len(), 0);
    }
    ...
    fn test_missing_parent_template() {
        let mut lysine = Lysine::default();
        assert_eq!(
            lysine.add_raw_template("a", "{% extends \"b\" %}").unwrap_err().to_string(),
    ...
    fn test_get_parent_blocks_definition() {
        ...
        let hey_definitions =
            lysine.get_template("child").unwrap().blocks_definitions.get("hey").unwrap();
        assert_eq!(hey_definitions.len(), 3);

        let ending_definitions =
            lysine.get_template("child").unwrap().blocks_definitions.get("ending").unwrap();
    ...
    fn test_get_parent_blocks_definition_nested_block() {
        ...
        let ending_definitions =
            lysine.get_template("parent").unwrap().blocks_definitions.get("ending").unwrap();
    ...
    fn test_can_autoescape_one_off_template() {
        ...
        let result = Lysine::one_off("{{ greeting }} world", &context, true).unwrap();
    ...
    fn test_can_disable_autoescape_one_off_template() {
        let mut context = Context::new();
        context.insert("greeting", &"<p>");
        let result = Lysine::one_off("{{ greeting }} world", &context, false).unwrap();
    ...
    fn test_set_escape_function() {
        ...
        let mut lysine = Lysine::default();
        lysine.add_raw_template("foo", "\"{{ content }}\"").unwrap();
        lysine.autoescape_on(vec!["foo"]);
        lysine.set_escape_fn(escape_c_string);
        let mut context = Context::new();
        context.insert("content", &"Hello\n\'world\"!");
        let result = lysine.render("foo", &context).unwrap();
    ...
    fn test_reset_escape_function() {
        ...
        lysine.add_raw_template("foo", "{{ content }}").unwrap();
        lysine.autoescape_on(vec!["foo"]);
        lysine.set_escape_fn(no_escape);
        lysine.reset_escape_fn();
        let mut context = Context::new();
        context.insert("content", &"Hello\n\'world\"!");
        let result = lysine.render("foo", &context).unwrap();
    ...
    fn test_value_one_off_template() {
        ...
        let result =
            Lysine::one_off("{{ greeting }} world", &Context::from_value(m).unwrap(), true).unwrap();
    ...
    fn test_render_str_with_custom_function() {
        let mut lysine = Lysine::default();
        lysine.register_function("echo", |args: &HashMap<_, JsonValue>| {
            Ok(args.get("greeting").map(JsonValue::to_owned).unwrap())
        });

        let result =
            lysine.render_str("{{ echo(greeting='Hello') }} world", &Context::default()).unwrap();
    ...
    fn test_render_map_with_dotted_keys() {
        let mut my_lysine = Lysine::default();
        my_lysine
        ...
        let mut lysine_context = Context::new();
        lysine_context.insert("map", &map);

        my_lysine.render("dots", &lysine_context).unwrap();
        my_lysine.render("urls", &lysine_context).unwrap();
    }
    ...
    fn test_extend_no_overlap() {
        ...
        let mut framework_lysine = Lysine::default();
        framework_lysine.add_raw_templates(vec![("four", "Framework X")]).unwrap();

        my_lysine.extend(&framework_lysine).unwrap();
        assert_eq!(my_lysine.templates.len(), 4);
        let result = my_lysine.render("four", &Context::default()).unwrap();
    ...
    fn test_extend_with_overlap() {
        ...
        my_lysine.extend(&framework_lysine).unwrap();
        assert_eq!(my_lysine.templates.len(), 4);
        let result = my_lysine.render("one", &Context::default()).unwrap();
    ...
    fn test_extend_new_filter() {
        let mut my_lysine = Lysine::default();
        let mut framework_lysine = Lysine::default();
        framework_lysine.register_filter("hello", |_: &JsonValue, _: &HashMap<String, JsonValue>| {
            Ok(JsonValue::Number(10.into()))
        });
        my_lysine.extend(&framework_lysine).unwrap();
        assert!(my_lysine.filters.contains_key("hello"));
    }
    ...
    fn test_extend_new_tester() {
        let mut my_lysine = Lysine::default();
        let mut framework_lysine = Lysine::default();
        framework_lysine.register_tester("hello", |_: Option<&JsonValue>, _: &[JsonValue]| Ok(true));
        my_lysine.extend(&framework_lysine).unwrap();
        assert!(my_lysine.testers.contains_key("hello"));
    }

    #[test]
    fn can_load_from_glob() {
        let lysine = Lysine::new("examples/basic/templates/**/*").unwrap();
        assert!(lysine.get_template("base.html").is_ok());
    }

    #[test]
    fn can_load_from_glob_with_patterns() {
        let lysine = Lysine::new("examples/basic/templates/**/*.{html, xml}").unwrap();
        assert!(lysine.get_template("base.html").is_ok());
    }

    #[test]
    fn full_reload_with_glob() {
        let mut lysine = Lysine::new("examples/basic/templates/**/*").unwrap();
        lysine.full_reload().unwrap();
    ...
    fn full_reload_with_glob_after_extending() {
        ...
        framework_lysine
            .add_raw_templates(vec![("one", "FRAMEWORK"), ("four", "Framework X")])
            .unwrap();
        lysine.extend(&framework_lysine).unwrap();
        lysine.full_reload().unwrap();

        assert!(lysine.get_template("base.html").is_ok());
        assert!(lysine.get_template("one").is_ok());
    }
    ...
    fn test_can_only_parse_templates() {
        let mut lysine = Lysine::parse("examples/basic/templates/**/*").unwrap();
        for tpl in lysine.templates.values_mut() {
    ...
    fn glob_work_with_absolute_paths() {
        ...
        let lysine = Lysine::new(&glob).expect("Couldn't build Lysine instance");
    ...
    fn glob_work_with_paths_starting_with_dots() {
        ...
        let scratch_dir = tempfile::Builder::new()
            .prefix("lysine_test_scratchspace")
            .tempdir_in(&this_dir)
            .expect(&format!(
    ...
    fn issues_found_fuzzing_expressions_are_fixed() {
        ...
        for (sample, expected_output) in samples {
            let res = Lysine::one_off(sample, &Context::new(), true);
    ...
    fn empty_list_on_invalid_glob() {
        ...
        println!("{:?}", lysine);
        assert!(lysine.is_ok());
        assert!(lysine.unwrap().templates.is_empty());
    }
}
