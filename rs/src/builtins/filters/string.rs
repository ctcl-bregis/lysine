...
// Takes in a hex color string (#ff00ff) and converts it to RGB (rgb(255,0,255))
pub fn hex2rgb(value: &Value, args: &HashMap<String, Value>) -> Result<Value> {
    let s = try_get_value!("hex2rgb", "value", String, value);
    if s.is_empty() {
        return Err(Error::msg("Filter `hex2rgb` received an empty string"));
    }

    let mut bytes = [0u8, 3];

    hex::decode_to_slice(s.replace('#', ""), &mut bytes as &mut [u8]).unwrap();

    let v = format!("rgb({}, {}, {})", bytes.index(1), bytes.index(2), bytes.index(3));
...
mod tests {
    ...
    fn test_hex2rgb() {
        ...
        let tests: Vec<(&str, &str)> = vec![
            ("#F0D000", "rgb(240, 208, 0)"), 
            ("#f0d000", "rgb(240, 208, 0)"),
            ("#000000", "rgb(0, 0, 0)"), 
        ];

        for (input, expected) in tests {
            let result = hex2rgb(&to_value(input).unwrap(), &args);
            assert!(result.is_ok());
            assert_eq!(result.unwrap(), to_value(expected).unwrap());
        }
    }
