...
use crate::parser::{Rule, LysineParser};

macro_rules! assert_lex_rule {
    ($rule: expr, $input: expr) => {
        let res = LysineParser::parse($rule, $input);
        println!("{:?}", $input);
        println!("{:#?}", res);
        if res.is_err() {
...
fn lex_ident() {
    ...
    assert!(LysineParser::parse(Rule::ident, "909").is_err());
}
...
fn lex_dotted_ident() {
    ...
    for i in invalid_inputs {
        assert!(LysineParser::parse(Rule::dotted_ident, i).is_err());
    }
}
...
fn lex_dotted_square_bracket_ident() {
    ...
    let invalid_inputs = vec![".", "9.w"];
    for i in invalid_inputs {
        assert!(LysineParser::parse(Rule::dotted_square_bracket_ident, i).is_err());
    }
}
...
fn lex_macro_definition() {
    ...
    for i in inputs {
        ...
        assert!(LysineParser::parse(Rule::macro_fn, i).is_ok());
    }
}
...
fn lex_test() {
    ...
    for i in inputs {
        // The () are not counted as tokens for some reasons so can't use the macro
        assert!(LysineParser::parse(Rule::test, i).is_ok());
    }
}

#[test]
fn lex_include_tag() {
    assert!(LysineParser::parse(Rule::include_tag, "{% include \"index.html\" %}").is_ok());
    assert!(LysineParser::parse(Rule::include_tag, "{% include [\"index.html\"] %}").is_ok());
    assert!(LysineParser::parse(Rule::include_tag, "{% include [\"index.html\"] ignore missing %}")
        .is_ok());
}

#[test]
fn lex_import_macro_tag() {
    assert!(LysineParser::parse(Rule::import_macro_tag, "{% import \"macros.html\" as macros %}",)
        .is_ok());
}

#[test]
fn lex_extends_tag() {
    assert!(LysineParser::parse(Rule::extends_tag, "{% extends \"index.html\" %}").is_ok());
}
...
fn lex_comment_tag() {
    ...
    for i in inputs {
        assert_lex_rule!(Rule::com_tag, i);
    }
}
...
#[test]
fn lex_else_tag() {
    assert!(LysineParser::parse(Rule::else_tag, "{% else %}").is_ok());
}
...
#[test]
fn lex_break_tag() {
    assert!(LysineParser::parse(Rule::break_tag, "{% break %}").is_ok());
}

#[test]
fn lex_continue_tag() {
    assert!(LysineParser::parse(Rule::continue_tag, "{% continue %}").is_ok());
}
...
fn lex_variable_tag() {
    ...
    for i in inputs {
        assert_lex_rule!(Rule::var_tag, i);
    }
}
...
fn lex_requires_whitespace_between_things() {
    ...
    for i in inputs {
        let res = LysineParser::parse(Rule::template, i);
        println!("{:?}", i);
        assert!(res.is_err());
    }
}
