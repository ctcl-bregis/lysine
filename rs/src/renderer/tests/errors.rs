...
fn error_location_basic() {
    ...
    lysine.add_raw_templates(vec![("tpl", "{{ 1 + true }}")]).unwrap();

    let result = lysine.render("tpl", &Context::new());
...
fn error_loading_macro_from_unloaded_namespace() {
    ...
    assert_eq!(
        ...
        "Macro namespace `macro` was not found in template `tpl`."
    );
}
...
fn error_location_base_template() {
    ...
    let result = lysine.render("child", &Context::new());
...
fn error_out_of_range_index() {
    let mut lysine = Lysine::default();
    lysine.add_raw_templates(vec![("tpl", "{{ arr[10] }}")]).unwrap();
...
fn error_unknown_index_variable() {
    let mut lysine = Lysine::default();
    lysine.add_raw_templates(vec![("tpl", "{{ arr[a] }}")]).unwrap();
    let mut context = Context::new();
    context.insert("arr", &[1, 2, 3]);
...
fn error_when_missing_macro_templates() {
    let mut lysine = Lysine::default();
    let result = lysine.add_raw_templates(vec![(
        "parent",
        "{% import \"macros\" as macros %}{{ macros::hello() }}{% block bob %}{% endblock bob %}",
    )]);
    assert_eq!(
        result.unwrap_err().to_string(),
        "Template `parent` loads macros from `macros` which isn\'t present in Lysine"
    );
}
...
fn error_when_using_variable_set_in_included_templates_outside() {
    ...
    let mut context = Context::new();
    context.insert("a", &10);
    let result = lysine.render("base", &context);
...
fn errors_with_inheritance_in_included_template() {
    ...
    let result = lysine.render("base", &Context::new());
...
fn error_string_concat_math_logic() {
    let mut lysine = Lysine::default();
    lysine.add_raw_templates(vec![("tpl", "{{ 'ho' ~ name < 10 }}")]).unwrap();
    let mut context = Context::new();
...
fn error_gives_source_on_tests() {
    let mut lysine = Lysine::default();
    lysine.add_raw_templates(vec![("tpl", "{% if a is undefined(1) %}-{% endif %}")]).unwrap();
    let result = lysine.render("tpl", &Context::new());
    println!("{:?}", result);
