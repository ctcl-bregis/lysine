...
fn can_remove_whitespace_basic() {
    ...
    for (input, expected) in inputs {
        ...
        lysine.add_raw_template("tpl", input).unwrap();
        println!("{} -> {:?}", input, expected);
        assert_eq!(lysine.render("tpl", &context).unwrap(), expected);
    }
}
...
fn can_remove_whitespace_include() {
    ...
    for (input, expected) in inputs {
        let mut lysine = Lysine::default();
        lysine.add_raw_templates(vec![("include", "Included"), ("tpl", input)]).unwrap();
        assert_eq!(lysine.render("tpl", &context).unwrap(), expected);
    }
}
...
fn works_with_filter_section() {
    ...
    let res = Lysine::one_off(input, &context, true).unwrap();
    assert_eq!(res, "  C   D");
}
