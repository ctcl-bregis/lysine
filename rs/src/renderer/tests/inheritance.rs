...
fn render_simple_inheritance() {
    ...
    .unwrap();
    let result = lysine.render("bottom", &Context::new());
...
fn render_nested_block_multiple_inheritance_no_super() {
    ...
    let result = lysine.render("page", &Context::new());
...
fn render_super_in_top_block_errors() {
    let mut lysine = Lysine::default();
    lysine.add_raw_templates(vec![("index", "{% block content%}{{super()}}{% endblock content %}")])
        .unwrap();
